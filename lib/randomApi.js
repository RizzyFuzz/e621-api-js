  ...
  return new Promise(async (resolve, reject) => {
    ...
    if (!username) { 
       reject({ 
         author: "RizzyFuzz",
         found: false, 
         message: `username no provided! >:3`
       }) 
     };
    ...
    if (!apikey) { 
       reject({ 
         author: "RizzyFuzz",
         found: false, 
         message: `api_key e621.net no providedno provided! >:3`
       }) 
     };
      ...
      const data = await axios({
        ...
        url: `https://e621.net/posts.json?login=${username}&api_key${apikey}`,
        headers: {
        "user-agent": "e621-api-js/1.5 (by rizzlydev on e621)",
     ...
     if (posts.length === 0) { 
      ...
      const result = sortedPosts.map(async post => {
        let buffer = await axios.get(`https://static1.e621.net/data/${post.file.md5.slice(0,2)}/${post.file.md5.slice(2,4)}/${post.file.md5}.${post.file.ext}` , { responseType: "arraybuffer" }).catch(() => ({ data: "" }));
        let urlnya = (await clph.tools.uploadFile(buffer)).result.url_file;
        return {
          author: "RizzyFuzz",
          post_id: post.id,
          found: true,
